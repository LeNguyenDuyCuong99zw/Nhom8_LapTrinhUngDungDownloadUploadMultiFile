...
class FlexTransferHub {
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.showNotification(
          `Chào mừng, ${this.currentUser.username}!`,
          "success"
        );
      ...
      this.showNotification(
        "Vui lòng đăng nhập tại Quản lý tệp để có thể tải lên",
        "warning"
      );
  ...
  showLoginPrompt() {
    ...
    prompt.innerHTML = `
      <div style="
        ...
        gap: 15px;
      ">
        <span>⚠️ Vui lòng đăng nhập để tải lên tệp</span>
  ...
  clearPreviousSessionFiles() {
    ...
    if (currentFiles === 0) {
      this.showNotification("Không có tệp nào để xóa", "info", 2000);
      return;
    }

    if (
      confirm(
        `Xóa tất cả ${currentFiles} tệp khỏi phiên tải lên hiện tại? (Các tệp được lưu an toàn và có thể quản lý trong Quản lý tệp)`
      )
    ) {
      this.transfers = [];
      // Clear sessionStorage as well
      sessionStorage.removeItem("current_session_files");
      console.log("🗑️ Đã xóa session storage");

      this.updateStatusCards();
      this.renderTransfers();
      this.showNotification(
        `Đã xóa ${currentFiles} tệp khỏi phiên hiện tại`,
        "success",
        3000
      );
    }
    ...
    try {
      ...
      this.ws.onopen = () => {
        ...
        if (this.wsConnectionAttempts > 0) {
          this.showNotification(
            "Đã kết nối lại tới máy chủ tải lên",
            "success"
          );
        }
      ...
      this.ws.onclose = () => {
        this.showNotification("Đã mất kết nối tới máy chủ tải lên", "error");
      ...
      this.ws.onerror = (error) => {
        ...
        this.showNotification(
          "Lỗi kết nối WebSocket - đang thử lại...",
          "warning"
        );
      };
    } catch (e) {
      ...
      this.showNotification(
        "Không thể kết nối tới máy chủ - sẽ thử lại sau 3s...",
        "error"
      );
      this.scheduleReconnection();
    }
  ...
  handleWSMessage(ev) {
    try {
      ...
      if (msg.event === "error") {
        ...
        this.showNotification(msg.error || "Lỗi tải lên", "error");
        this.renderTransfers();
        // Thử start các transfer đang chờ khác
        this.maybeStartNextUploads();
      }
      ...
      if (msg.event === "download-complete") {
        ...
        this.renderTransfers();
        this.showNotification(
          `Tải xuống hoàn tất: ${transfer.name}`,
          "success"
        );
      }
      ...
      if (msg.event === "download-error") {
        ...
        transfer.status = "error";
        this.showNotification(`Tải xuống thất bại: ${msg.error}`, "error");
        this.renderTransfers();
      }
    } catch (error) {
  ...
  async uploadLoop(transfer) {
    ...
    try {
      ...
      if (!transfer.file) {
        ...
        transfer.status = "error";
        transfer.error =
          "Không tìm thấy đối tượng tệp. Vui lòng thử tải lại tệp.";
        this.renderTransfers();
        return;
      }
      ...
      while (
        ...
        if (waitCount >= maxWaitTime) {
          ...
          if (this.ws.readyState !== WebSocket.OPEN) {
            ...
            transfer.status = "paused";
            this.showNotification(
              `Mất kết nối với ${transfer.name}. Nhấn 'Tiếp tục' để thử lại.`,
              "warning"
            );
          } else {
            transfer.status = "error";
  ...
  retryUpload(transfer) {
    ...
    if (transfer.retryCount > 3) {
      this.showNotification(
        `Tải lên thất bại sau 3 lần thử: ${transfer.name}`,
        "error"
      );
      return;
    }
    ...
    this.showNotification(
      `Thử tải lại (lần ${transfer.retryCount}): ${transfer.name}`,
      "info"
    );
    this.maybeStartNextUploads();
  }
  ...
  setupSettingsModal() {
    ...
    applySettings.addEventListener("click", () => {
      ...
      this.showNotification(
        ...
        } tải lên đồng thời, ${Math.floor(this.chunkSize / 1024)}KB mỗi khối`,
        "success"
      );
      this.updateStatusCards();
      settingsModal.style.display = "none";
  ...
  addDownloadURL() {
    ...
    if (!url) {
      this.showNotification("Vui lòng nhập URL hợp lệ", "error");
      return;
    }

    if (!this.isValidURL(url)) {
      this.showNotification("Vui lòng nhập định dạng URL hợp lệ", "error");
      return;
    }
    ...
    this.updateStatusCards();
    this.renderTransfers();
    this.showNotification("Đã thêm tải xuống vào hàng đợi", "success");
  }
  ...
  createTransferSection(title, transfers, sectionClass) {
    ...
    header.innerHTML = `
      <h3>${title}</h3>
      <span class="section-count">${transfers.length} tệp</span>
    `;
  ...
  createNewTransfersList(filteredTransfers) {
    ...
    const listClass =
      this.viewMode === "grid" ? "transfers-grid" : "transfers-list";
    transfersList.className = listClass;
    transfersList.setAttribute("aria-label", "Danh sách truyền");
  ...
  getActionButtonsHTML(transfer) {
    if (transfer.status === "completed") {
      ...
      return `<span class="status-text completed">✓ Đã hoàn thành</span>`;
    } else if (transfer.status === "uploading") {
      // Đang upload lên remote server, chỉ hiển thị status
      return `<span class="status-text">⬆ Đang tải lên máy chủ...</span>`;
    } else if (transfer.status === "completing") {
      // Đang finalize file local, chỉ hiển thị status
      return `<span class="status-text">⚡ Đang hoàn tất...</span>`;
    } else if (transfer.status === "pending") {
      return `
        <button class="action-btn start-btn" title="Bắt đầu">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
            <path d="M8 5v14l11-7z"/>
          </svg>
        </button>
        <button class="action-btn stop-btn" title="Dừng">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
      ...
      return `
        <button class="action-btn pause-btn" title="Đang bắt đầu..." disabled>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
      ...
      return `
        <button class="action-btn pause-btn" title="Tạm dừng">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
            <path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/>
          </svg>
        </button>
      ...
      return `
        <button class="action-btn resume-btn" title="Tiếp tục">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
            <path d="M8 5v14l11-7z"/>
          </svg>
        </button>
      ...
      return `
        <button class="action-btn resume-btn" title="Đang tiếp tục..." disabled>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
            <path d="M8 5v14l11-7z"/>
          </svg>
        </button>
      ...
      return `
        <button class="action-btn start-btn" title="Thử lại">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
      ...
      return `
        <button class="action-btn start-btn" title="Khởi động lại">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
            <path d="M8 5v14l11-7z"/>
          </svg>
        </button>`;
  ...
  restartTransfer(transfer) {
    ...
    if (transfer.type === "upload") {
      ...
      if (transfer.isPreviousSession || transfer.status === "completed") {
        ...
        if (!transfer.file) {
          this.showNotification(
            "Vui lòng chọn lại tệp để tải lên lại",
            "warning"
          );
        ...
        this.showNotification(
          `Đã thêm ${transfer.name} vào hàng đợi tải lên`,
          "success"
        );
        this.renderTransfers();
        this.maybeStartNextUploads();
        ...
        if (!transfer.file) {
          ...
          this.showNotification(
            `Đang khởi động lại tải lên: ${transfer.name}`,
            "info"
          );
          return;
        } else {
      ...
      this.showNotification(
        `Đang khởi động lại tải xuống: ${transfer.name}`,
        "info"
      );
      this.renderTransfers();
  ...
  triggerFilePickerForRestart(originalTransfer, newTransfer) {
    ...
    input.onchange = (e) => {
      ...
      if (
        ...
        this.transfers.push(newTransfer);
        this.showNotification(
          `Đã thêm ${file.name} vào hàng đợi tải lên`,
          "success"
        );
        this.renderTransfers();
        this.maybeStartNextUploads();
  ...
  async removeTransfer(transfer) {
    ...
    if (!["completed", "stopped", "error"].includes(transfer.status)) {
      this.showNotification(
        "Chỉ có thể xóa tác vụ đã hoàn thành, đã dừng hoặc bị lỗi",
        "warning"
      );
      return;
    }
    ...
    if (transfer.isPreviousSession || transfer.status === "completed") {
      const confirmMsg = `Xóa "${transfer.name}"?\n\nTệp sẽ được chuyển vào thùng rác. Bạn có thể khôi phục sau nếu cần.`;
    ...
    if (transfer.isPreviousSession && transfer.fileId) {
      try {
        ...
        this.showNotification(
          `Đã chuyển "${transfer.name}" vào thùng rác`,
          "success"
        );
      } catch (error) {
    ...
    if (index > -1) {
      ...
      if (!transfer.isPreviousSession || !transfer.fileId) {
        this.showNotification(
          `Đã xóa "${transfer.name}" khỏi danh sách`,
          "success"
        );
      }
      ...
      this.showNotification("Không tìm thấy tác vụ", "error");
    }
  }
